setwd("/Users/kianamac/Dropbox (UFL)/courses/Spring 2019/MultivariateDataAnalysis/HW/hw3/region/")
library(MASS)
library(caret)
library(rpart.plot)
#------------------------------------- Read Files------------------------------------------------------------------
...
LDA_model <- train(region ~ palmitic+palmitoleic+stearic+oleic+linoleic+linolenic+arachidic+eicosenoic , data = data_train, method = "lda",trControl = train.control)
QDA_model <-train(region ~ palmitic+palmitoleic+stearic+oleic+linoleic+linolenic+arachidic+eicosenoic , data = dataset, method = "qda",trControl = train.control) 
DT_model <- train(region ~palmitic+palmitoleic+stearic+oleic+linoleic+linolenic+arachidic+eicosenoic, data = data_train, method = "rpart",trControl = train.control)
RF_model <- randomForest(region ~palmitic+palmitoleic+stearic+oleic+linoleic+linolenic+arachidic+eicosenoic, data = data_train,ntree = 33, method = "rf",trControl = train.control, importance= TRUE)
KNN_model <-train(region ~palmitic+palmitoleic+stearic+oleic+linoleic+linolenic+arachidic+eicosenoic, data = data_train, method = "knn",trControl = train.control,tuneGrid = expand.grid(k = 1:25))
SVM_model <-train(region ~palmitic+palmitoleic+stearic+oleic+linoleic+linolenic+arachidic+eicosenoic, data = data_train, method = "svmLinear",trControl = train.control)
Naive_model <- train(region ~palmitic+palmitoleic+stearic+oleic+linoleic+linolenic+arachidic+eicosenoic, data = data_train, method = "naive_bayes",trControl = train.control)
...
cm_DT <- confusionMatrix(DT_predict,data_test$region)
cm_RF <- confusionMatrix(RF_predict,data_test$region)
cm_KNN <- confusionMatrix(KNN_predict,data_test$region)
cm_SVM <- confusionMatrix(SVM_predict,data_test$region)
cm_Naive <- confusionMatrix(Naive_predict,data_test$region)
cm_LDA <- confusionMatrix(LDA_predict,data_test$region)
cm_QDA <- confusionMatrix(QDA_predict,data_test$region)
#-------------------------------------------- write into files------------------------------------------------------
